...
static struct gpio_desc *my_led2 = NULL;
		...
		case '1':
			{
				...
				gpiod_set_value(my_led2, user_buffer[0] - '0');
				break;
			}
...
static int dt_probe(struct platform_device *pdev) {
	...
	if(IS_ERR(my_led)) {
		printk("dt_gpio - Error! Could not setup the GPIO for my_led\n");
		return -1 * IS_ERR(my_led);
	}
	/* Init GPIO */
	
	my_led2 = gpiod_get(dev, "red-led", GPIOD_OUT_LOW);
	if(IS_ERR(my_led2)) {
		printk("dt_gpio - Error! Could not setup the GPIO for my_led2\n");
		return -1 * IS_ERR(my_led2);
	}
